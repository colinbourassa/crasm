...
#include "dcl.h"
#include "label.h"
extern unsigned long pc,codelen;
extern int segment;
...
result(void)
{
	extern int errnumber,warnnumber;
	char buffer[40];
	...
	output(buffer); outputEOL();
	sprintf(buffer,"WARNINGS:  %4d",warnnumber);
	output(buffer); outputEOL(); outputEOL();
	if ( !errnumber )
